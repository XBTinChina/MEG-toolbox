function [chl_avr_comodulation ] = TX_PhasePowerCoupling( ft_cell,trl_num,channls,time_range)
...
for chl = 1:length(channls)
    ...
    for trl = 1:trl_num
        ...
        AmpFreqTransformed = zeros(length(AmpFreqVector), length(lfp(real_data)));
        PhaseFreqTransformed = zeros(length(PhaseFreqVector), length(lfp(real_data)));
        ...
        for ii=1:length(AmpFreqVector)
            ...
            AmpFreqTransformed(ii, :) = temp_amp(real_data);
        end
        ...
        for jj=1:length(PhaseFreqVector)
            ...
            PhaseFreqTransformed(jj, :) = temp_pha(real_data);
        end
        toc
